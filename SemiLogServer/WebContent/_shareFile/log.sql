...
CREATE TABLE `ACTION` (
	...
	`KOR_NAME`   VARCHAR(50)  NULL     COMMENT '한글이름', -- 한글이름
	`DEF_MINUTE` INTEGER      NULL     COMMENT '기본설정시간', -- 기본설정시간
...
CREATE TABLE `LOG` (
	...
	`ACTION_ID`  INTEGER NULL     COMMENT '일번호', -- 일번호
...
-- 기록
ALTER TABLE `LOG`
	ADD CONSTRAINT `FK_ACTION_TO_LOG` -- 일 -> 기록
		FOREIGN KEY (
			`ACTION_ID` -- 일번호
		)
		REFERENCES `ACTION` ( -- 일
